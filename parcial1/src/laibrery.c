...
static int getFloat(float *pResultado)
{
	...
	if (pResultado != NULL && myGets(bufferString, sizeof(bufferString)) == 0 && esFlotante(bufferString, sizeof(bufferString)))
	{
		retorno = 0;
...
int getAlfanumerico(char array[], char *mensaje, char *errorMensaje, int tamano, int reintentos)
{
	...
	//valido
	if (array != NULL && mensaje != NULL && errorMensaje != NULL && tamano >= 1 && reintentos >= 0)
	{
...
int getCadenaTelefonica(char array[], char *mensaje, char *errorMensaje, int tamano, int reintentos)
{
	...
	if (array != NULL && mensaje != NULL && errorMensaje != NULL && tamano >= 1 && reintentos >= 0)
	{
		...
		if (retorno == 1)
		{
			...
			strncpy(array, bufferChar, sizeof(bufferChar));
			//inPut = bufferChar;
		}
...
int esLetraConEspacio(char array[], char *mensaje, char *errorMensaje, int tamano, int reintentos)
{
	...
	if (array != NULL && mensaje != NULL && errorMensaje != NULL && tamano >= 1 && reintentos >= 0)
	{
		...
		if (retorno == 1)
		{
			strncpy(array, bufferChar, strlen(bufferChar));
			//inPut = bufferChar;
		}
